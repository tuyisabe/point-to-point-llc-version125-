...
export default function BookedCabScreen(props) {
            ...
            setTimeout(() => {
              props.navigation.dispatch(
                CommonActions.reset({
                  index: 0,
                  routes: [
                    { name: "PaymentDetails", params: { booking: booking } },
                  ],
                })
              );
            }, 1000);
  ...
  const renderButtons = () => {
    return (curBooking &&
      ...
      <View style={{ flex: 1.5, flexDirection: isRTL ? "row-reverse" : "row" }}>
        ...
        {role == "driver" &&
        (!appConsts.captureBookingImage ||
          (curBooking.pickup_image && appConsts.captureBookingImage) ||
          (!settings.AllowDeliveryPickupImageCapture &&
            appConsts.captureBookingImage)) &&
        (curBooking.status == "ACCEPTED" || curBooking.status == "ARRIVED") ? (
          <View style={{ flex: 1 }}>
            <Button
              ...
              buttonStyle={{
                height: "100%",
                backgroundColor: colors.START_TRIP,
              }}
              containerStyle={{ height: "100%" }}
            />
          </View>
        ...
        {role == "driver" &&
        (!appConsts.captureBookingImage ||
          (curBooking.deliver_image && appConsts.captureBookingImage) ||
          (!settings.AllowFinalDeliveryImageCapture &&
            appConsts.captureBookingImage)) &&
        curBooking.status == "STARTED" ? (
          <View style={{ flex: 1 }}>
  ...
  const acceptBid = (item) => {
    ...
    if (
      (bookingObj.payment_mode === "wallet" &&
        parseFloat(auth.profile.walletBalance) >= item.trip_cost) ||
      bookingObj.payment_mode === "cash" ||
      bookingObj.payment_mode === "card"
    ) {
                    ...
                    <View key={key} style={styles.vew}>
                      ...
                      >
                        ...
                        >
                          <Image
                            source={
                              curBooking &&
                              curBooking.driverOffers[key].driver_image
                                ? {
                                    uri: curBooking.driverOffers[key]
                                      .driver_image,
                                  }
                        ...
                        <View style={{ width: "75%", alignItems: "center" }}>
                          ...
                          <StarRating
                            ...
                            rating={
                              curBooking &&
                              curBooking.driverOffers[key] &&
                              curBooking.driverOffers[key].driverRating
                                ? parseFloat(
                                    curBooking.driverOffers[key].driverRating
                                  )
                                : 0
                      ...
                      >
                        ...
                        >
                          ...
                          curBooking.driverEstimates[key].timein_text
                            ? curBooking.driverEstimates[key].timein_text
                            : t("within_min")}
                        </Text>
                      </View>
  ...
  return (
    <View style={styles.mainContainer}>
          ...
          (new Date(curBooking.tripdate) - new Date()) / (1000 * 60) > 15 ? (
            ...
            >
              ...
              <Text style={{ fontWeight: "bold", fontSize: 16 }}>
                {moment(curBooking.tripdate).format("MMM d,Y, hh:mm A")}
              </Text>
            </View>
          ) : null}
      ...
      <OtpModal
        ...
        onMatch={(value) =>
          value ? (appConsts.hasStartOtp ? startBooking() : endBooking()) : null
        }
      />
    </View>
  );
