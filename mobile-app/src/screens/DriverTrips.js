...
export default function DriverTrips(props) {
  ...
  const onPressAccept = (item, price) => {
    ...
    if (
      ...
      if (
        ...
        !settings.disable_cash &&
        (wallet_balance <= 0 ||
          (wallet_balance > 0 && wallet_balance < item.convenience_fees)) &&
        ...
        if (parseFloat(price) > 0) {
          ...
          let convenience_fees =
            item.commission_type == "flat"
              ...
              : (parseFloat(price) * parseFloat(item.commission_rate)) / 100;
          let obj = {};
          ...
          obj.driver_share = parseFloat(
            parseFloat(price) - parseFloat(convenience_fees)
          ).toFixed(2);
          obj.car_image = profile.car_image ? profile.car_image : "";
          ...
          if (item.customer_token) {
            RequestPushMsg(item.customer_token, {
        ...
        setTimeout(() => {
          props.navigation.dispatch(
            CommonActions.reset({
              index: 0,
              routes: [{ name: "BookedCab", params: { bookingId: item.id } }],
            })
          );
        }, 3000);
      }
  ...
  const goToBooking = (id) => {
    props.navigation.dispatch(
      CommonActions.reset({
        index: 0,
        routes: [{ name: "BookedCab", params: { bookingId: id } }],
      })
    );
  };
  ...
  const navEditUser = () => {
    props.navigation.dispatch(
      CommonActions.reset({
        index: 0,
        routes: [{ name: "editUser", params: { fromPage: "DriverTrips" } }],
      })
    );
  };
  ...
  return (
    <View style={styles.mainViewStyle}>
      ...
      >
        <FlatList
          data={
            auth.profile && auth.profile.uid && auth.profile.driverActiveStatus
              ? appConsts && appConsts.showBookingOptions
                ? tasks
                  ? tasks
                  : activeBookings
                : auth.profile.queue
                ...
                <View
                  style={{
                    ...
                    height:
                      10 +
                      (gps.error ? 70 : 0) +
                      (!checks.driverActiveStatus ? 70 : 0) +
                      (!checks.carExists && settings.carType_required
                        ? 70
                        : 0) +
                      (!checks.carApproved && settings.carType_required
                      ...
                      !checks.imageIdApproval
                        ? 70
                        : 0) +
                      (!checks.term && settings.term_required ? 70 : 0),
                  }}
                >
                  {gps.error ? (
                    <View
                ...
                <View style={styles.mapDetails}>
                  <View style={styles.dateView}>
                    <View>
                      <Text style={styles.listDate}>
                        {moment(item.tripdate).format("MMM d,Y, hh:mm A")}
                      </Text>
                    </View>
                    <RateView
                  ...
                  {item && item.status != "NEW" ? (
                    ...
                    >
                      ...
                      {!appConsts.acceptWithAmount ||
                      (appConsts.acceptWithAmount &&
                        !(
                          item.driverOffers &&
                          item.driverOffers[auth.profile.uid]
                        )) ? (
