...
export default function SettingsScreen(props) {
  ...
  const menuList = [
    ...
    {
      name:
        auth.profile && auth.profile && auth.profile.usertype == "driver"
          ? t("convert_to_rider")
          : t("convert_to_driver"),
  ...
  const convert = () => {
    Alert.alert(
      t("convert_button"),
      auth.profile && auth.profile.usertype == "driver"
        ? t("convert_to_rider")
        : t("convert_to_driver"),
      [
        {
          ...
          onPress: async () => {
            let userData = {
              approved:
                auth.profile && auth.profile.usertype == "driver"
                  ? true
                  : settings && settings.driver_approval
                  ? false
                  : true,
              usertype:
                auth.profile && auth.profile.usertype == "driver"
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          if (
            ...
            auth.profile &&
            auth.profile.usertype == "customer" &&
            item.navigationName == "editUser" &&
            !(
              settings &&
              ((settings.bank_fields && settings.RiderWithDraw) ||
